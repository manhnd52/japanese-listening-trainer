...
export const moveAudio = async (
  ...
  try {
    ...
    if (!folderId)
      return res
        .status(400)
        .json({ success: false, message: "folderId is required" });
    if (!userId)
      return res
        .status(400)
        .json({ success: false, message: "userId is required" });
    ...
    const movedAudio = await audioService.moveAudio(
      Number(id),
      Number(folderId)
    );
    res.json({
...
export const toggleFavorite = async (
  ...
  try {
    ...
    const updated = await audioService.toggleFavorite(
      Number(id),
      Number(userId)
    );
    if (!updated)
      return res
        ...
        .json({
          success: false,
          message: "Audio not found or not owned by user",
        });
    res
      .status(200)
      .json({ success: true, message: "Favorite updated", data: updated });
  } catch (err) {
    next(err);
  }
};
...
export const getRecentlyListened = async (
  ...
  try {
    ...
    const audios = await audioService.getRecentlyListened(
      Number(userId),
      limitNum
    );
    res.json({ success: true, data: audios });
...
export const incrementListenCount = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const audioStats = await audioService.incrementListenCount(
      Number(id),
      Number(userId)
    );
    await audioService.updateUserDailyListenTime(
      Number(userId),
      audio.duration || 0
    );
...
export const getRandomAudiosFromMyList = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const limitNum = limit ? Math.min(parseInt(limit as string), 50) : 10;
    const audios = await audioService.getRandomAudiosFromMyList(
      Number(userId),
      limitNum
    );
    res.json({ success: true, data: audios });
...
export const getRandomAudiosFromCommunity = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const limitNum = limit ? Math.min(parseInt(limit as string), 50) : 10;
    const audios = await audioService.getRandomAudiosFromCommunity(
      Number(userId),
      limitNum
    );
    res.json({ success: true, data: audios });
