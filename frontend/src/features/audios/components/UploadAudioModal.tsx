...
const UploadAudioModal: React.FC<UploadAudioModalProps> = ({
  isOpen,
  onClose,
}) => {
  const dispatch = useAppDispatch();
  const { folders, loading, error, uploadProgress } = useAppSelector(
    (state) => state.audio
  ); // ✅ lấy uploadProgress
  ...
  return (
    <div
      className="fixed inset-0 z-[999] flex items-center justify-center bg-black/20 backdrop-blur-sm animate-fade-in"
      onClick={handleClose}
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl m-4 border border-brand-200 max-h-[90vh] overflow-y-auto"
